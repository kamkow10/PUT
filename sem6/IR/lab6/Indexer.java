...
public class Indexer
{
    ...
    private Document getHTMLDocument(String path, int id)
    {
        ...
        */
        ...
        // ----------------------------------
        StoredField sf = new StoredField(Constants.id, id);
        // ----------------------------------
        ...
        // ----------------------------------
        String text = getTextFromHTMLFile(file);
        TextField tf = new TextField(Constants.content, text, Field.Store.NO);
        // ----------------------------------

        // TODO create a field that is stored and indexed
        // and contains file name
        // ----------------------------------
        TextField tf2 = new TextField(Constants.filename, file.getName(), Field.Store.YES);
        // ----------------------------------

        // TODO create an INT field (IntPoint) that is indexed
        // and contains file size (bytes, .length())
        // ----------------------------------
        IntPoint ip = new IntPoint(Constants.filesize, (int)(file.length()));
        // ----------------------------------
        // //TODO IntPoint is not stored but we want to a file size
        // ... so add another field (StoredField) that stores file size
        // ----------------------------------
        StoredField sf2 = new StoredField(Constants.filesize, file.length());
        // ----------------------------------
